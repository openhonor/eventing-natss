...
func TestNewConfigWatcher_defaults(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			expectNatssContains:   `"SaramaYamlString":"{Version: 2.0.0}"`,
		},
		{
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			assert.NotNil(t, cw.NatssConfig(), "natss config should be enabled")
			...
			assert.Equal(t, EnvNatssCfg, envs[3].Name, "third env var is natss config")
			assert.Contains(t, envs[3].Value, tc.expectNatssContains)
		})
	}
}
...
func configMapWatcherWithSampleData() configmap.Watcher {
	return configmap.NewStaticWatcher(
		...
		newTestConfigMap(NatssConfigMapName(), natssConfigMapData()),
	)
}
...
func configMapWatcherWithEmptyData() configmap.Watcher {
	return configmap.NewStaticWatcher(
		...
		newTestConfigMap(NatssConfigMapName(), nil),
	)
}
...
func natssConfigMapData() map[string]string {
	return map[string]string{
		"sarama": `{Version: 2.0.0}`,
	}
}

func TestNatssConfigToJSON(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			output, err := natssConfigToJSON(&tc.cfg)
			if tc.success == true && err != nil {
				t.Errorf("NatssConfig should've been converted to JSON successfully. Err: %e", err)
			}
			if tc.success == false && err != nil {
				t.Errorf("NatssConfig should not have been converted to JSON successfully.")
			}
			if output != tc.expected {
				t.Errorf("NatssConfig conversion is wrong, want: %s vs got %s", tc.expected, output)
			}
		})
	}
